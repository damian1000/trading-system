...
import com.damianhoward.positionledger.position.View
import com.damianhoward.positionledger.position.ViewTotals
...
class ReadinessTest {
    ...
    fun `metrics render the same snapshot readiness evaluates`() {
        ...
        assertTrue(metrics.contains("position_ledger_ready 1"), metrics)
        assertTrue(metrics.contains("position_ledger_database_up 1"), metrics)
        assertTrue(metrics.contains("""position_ledger_consumer_up{consumer="fills"} 1"""), metrics)
        assertTrue(metrics.contains("position_ledger_view_offset{view=\"positions\"} 9"), metrics)
        assertTrue(metrics.contains("position_ledger_views_coherent 1"), metrics)
        assertTrue(metrics.contains("""position_ledger_ledger_agrees{view="positions"} 1"""), metrics)
        assertTrue(metrics.contains("""position_ledger_ledger_agrees{view="exposure"} 1"""), metrics)
        assertTrue(metrics.contains("""position_ledger_ledger_divergent_symbols{view="position_book"} 0"""), metrics)
        assertTrue(metrics.contains("position_ledger_dead_letters_published_total 3"), metrics)
        assertTrue(metrics.contains("position_ledger_dead_letters_failed_total 1"), metrics)
    ...
    fun `metrics carry heap against its ceiling, which is what sizes the ceiling`() {
        ...
        assertTrue(metrics.contains("position_ledger_jvm_heap_used_bytes"), metrics)
        assertTrue(metrics.contains("position_ledger_jvm_heap_max_bytes"), metrics)
        assertTrue(metrics.contains("position_ledger_process_uptime_seconds"), metrics)
    }
    ...
    fun `metrics count divergences rather than labelling by symbol`() {
        ...
        val metrics = readiness().metrics()
        assertTrue(metrics.contains("""position_ledger_ledger_agrees{view="positions"} 0"""), metrics)
        assertTrue(metrics.contains("""position_ledger_ledger_divergent_symbols{view="positions"} 1"""), metrics)
    ...
    fun `a breach leaves the process as a count an alert can fire on`() {
        ...
        assertTrue(metrics.contains("position_ledger_exposure_breaches_total 3"), metrics)
        assertTrue(metrics.contains("position_ledger_exposure_symbols_breached 1"), metrics)
        assertTrue(metrics.contains("""position_ledger_exposure_utilisation_ratio{limit="position"} 1.2000"""), metrics)
        assertTrue(metrics.contains("""position_ledger_exposure_utilisation_ratio{limit="notional"} 0.4000"""), metrics)
    }
    ...
    fun `utilisation is labelled by ceiling, never by symbol`() {
        ...
        assertTrue(metrics.contains("""position_ledger_exposure_utilisation_ratio{limit="position"} 0.9000"""), metrics)
        assertTrue(metrics.contains("""position_ledger_exposure_utilisation_ratio{limit="notional"} 0.8000"""), metrics)
    ...
    fun `no exposure yet publishes no utilisation, but still counts breaches`() {
        ...
        assertFalse(metrics.contains("position_ledger_exposure_utilisation_ratio"), metrics)
        assertTrue(metrics.contains("position_ledger_exposure_breaches_total 0"), metrics)
    }
    ...
    fun `metrics carry no free text, because the endpoint is unauthenticated`() {
        ...
        assertTrue(metrics.contains("""position_ledger_consumer_up{consumer="fills"} 0"""), metrics)
    }
    ...
    fun `an inconclusive view publishes no verdict rather than a zero`() {
        ...
        assertFalse(metrics.contains("""position_ledger_ledger_agrees{view="exposure"}"""), metrics)
        assertTrue(metrics.contains("""position_ledger_ledger_check_inconclusive{view="exposure"} 1"""), metrics)
        assertTrue(metrics.contains("""position_ledger_ledger_check_inconclusive{view="positions"} 0"""), metrics)
        assertTrue(metrics.contains("""position_ledger_ledger_agrees{view="positions"} 1"""), metrics)
    }
